...
public final class PathFindBehavior2 implements PolygonalMap2.IPathfinder {
   ...
   private static final ObjectPool actualPool = new ObjectPool(DebugPt::new);
   private Goal goal;
   ...
   public BehaviorResult update() {
      if (this.chr.getFinder().progress == AStarPathFinder.PathFindProgress.notrunning) {
         ...
         if (Core.bDebug && DebugOptions.instance.PathfindRenderPath.getValue() && this.chr instanceof IsoPlayer) {
            this.actualPos.add(((DebugPt)actualPool.alloc()).init(this.chr.x, this.chr.y, this.chr.z, var1 == ClimbOverFenceState.instance() || var1 == ClimbThroughWindowState.instance()));
         }
   ...
   public static void closestPointOnPath(float var0, float var1, float var2, IsoMovingObject var3, PolygonalMap2.Path var4, PointOnPath var5) {
   ...
   private static enum Goal {
      ...
      // $FF: synthetic method
      private static Goal[] $values() {
         return new Goal[]{None, Character, Location, Sound, VehicleAdjacent, VehicleArea, VehicleSeat};
      }
   }
   ...
   public static enum BehaviorResult {
      ...
      // $FF: synthetic method
      private static BehaviorResult[] $values() {
         return new BehaviorResult[]{Working, Failed, Succeeded};
      }
   }
